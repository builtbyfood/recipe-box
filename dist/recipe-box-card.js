 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function ge(e){return he({...e,state:!0,attribute:!1})}function ue(e){if(!e)return"";const t=e.match(/^PT(?:(\d+)H)?(?:(\d+)M)?$/);if(!t)return"";const i=parseInt(t[1]||"0",10),o=parseInt(t[2]||"0",10);return i&&o?`${i}h ${o}m`:i?`${i}h`:o?`${o}m`:""}function me(e){if(!e)return"never";const t=new Date(e).getTime();if(isNaN(t))return"never";const i=(Date.now()-t)/1e3;return i<60?"just now":i<3600?`${Math.floor(i/60)}m ago`:i<86400?`${Math.floor(i/3600)}h ago`:i<604800?`${Math.floor(i/86400)}d ago`:i<2592e3?`${Math.floor(i/604800)}w ago`:i<31536e3?`${Math.floor(i/2592e3)}mo ago`:`${Math.floor(i/31536e3)}y ago`}function _e(e){if(!e)return null;const t=/^PT(?:(\d+)H)?(?:(\d+)M)?$/.exec(e);if(!t)return null;return 60*parseInt(t[1]||"0",10)+parseInt(t[2]||"0",10)}let ve=class extends ne{constructor(){super(...arguments),this.recipes=[],this.columns=3,this.showFilters=!1,this.groupBy="none",this._search="",this._activeTags=new Set,this._activeChips=new Set,this._sortBy="name",this._viewMode="grid"}connectedCallback(){super.connectedCallback();const e=localStorage.getItem("recipe-box-group-by");e&&["none","tag","category","source"].includes(e)&&(this._groupOverride=e);const t=localStorage.getItem("recipe-box-sort-by");t&&["name","imported","last_cooked","cooked_count","total_time"].includes(t)&&(this._sortBy=t);const i=localStorage.getItem("recipe-box-view-mode");"grid"!==i&&"list"!==i||(this._viewMode=i)}get _activeGroupBy(){return this._groupOverride??this.groupBy}_setGroupBy(e){this._groupOverride=e,localStorage.setItem("recipe-box-group-by",e)}_setSortBy(e){this._sortBy=e,localStorage.setItem("recipe-box-sort-by",e)}_setViewMode(e){this._viewMode=e,localStorage.setItem("recipe-box-view-mode",e)}_toggleChip(e){this._activeChips.has(e)?this._activeChips.delete(e):this._activeChips.add(e),this._activeChips=new Set(this._activeChips)}_selectRecipe(e){this.dispatchEvent(new CustomEvent("recipe-selected",{detail:{slug:e},bubbles:!0,composed:!0}))}_allTags(){const e=new Set;for(const t of this.recipes)for(const i of t.tags)e.add(i);return[...e].sort()}_toggleTag(e){this._activeTags.has(e)?this._activeTags.delete(e):this._activeTags.add(e),this._activeTags=new Set(this._activeTags)}_filtered(){const e=this._search.trim().toLowerCase(),t=Date.now()-12096e5,i=this.recipes.filter(i=>{if(e){const t=i.name.toLowerCase().includes(e),o=(i.ingredients_text||"").toLowerCase().includes(e);if(!t&&!o)return!1}if(this._activeTags.size>0)for(const e of this._activeTags)if(!i.tags.includes(e))return!1;for(const e of this._activeChips){if("never"===e&&i.cooked_count>0)return!1;if("favorite"===e&&i.cooked_count<3)return!1;if("quick"===e){const e=_e(i.totalTime);if(null===e||e>30)return!1}if("recent"===e){if(!i.last_cooked)return!1;if(new Date(i.last_cooked).getTime()<t)return!1}}return!0});return this._sorted(i)}_sorted(e){const t=[...e];switch(this._sortBy){case"imported":t.sort((e,t)=>(t.imported_at??"").localeCompare(e.imported_at??""));break;case"last_cooked":t.sort((e,t)=>e.last_cooked||t.last_cooked?e.last_cooked?t.last_cooked?t.last_cooked.localeCompare(e.last_cooked):-1:1:e.name.localeCompare(t.name));break;case"cooked_count":t.sort((e,t)=>(t.cooked_count||0)-(e.cooked_count||0)||e.name.localeCompare(t.name));break;case"total_time":t.sort((e,t)=>{const i=_e(e.totalTime),o=_e(t.totalTime);return null===i&&null===o?e.name.localeCompare(t.name):null===i?1:null===o?-1:i-o});break;default:t.sort((e,t)=>e.name.localeCompare(t.name))}return t}_cookAgain(){return[...this.recipes].filter(e=>e.cooked_count>0).sort((e,t)=>(t.cooked_count||0)-(e.cooked_count||0)).slice(0,8)}_pickRandom(){const e=this._filtered();if(0===e.length)return;const t=e[Math.floor(Math.random()*e.length)];this._selectRecipe(t.slug)}render(){const e=this._filtered(),t=this._allTags(),i=this._cookAgain();return F`
